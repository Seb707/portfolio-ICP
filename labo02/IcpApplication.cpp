...
void IcpApplication::stepIcp()
{
  ...
  // plus près de B[:,i]
  // 
  gti320::Matrix4d tmResult;
  tmResult.setIdentity();
  ...
  for (int i = 0; i < npoints; ++i)
    {
      ...
      gti320::Vector<double, 3> sourcePoint(m_sourcePoints(0, i), m_sourcePoints(1, i), m_sourcePoints(2, i));

      result = m_srcTM * m_initTM * sourcePoint;
      // TODO Étape 1.2 trouver le point de m_refPoints qui est le plus près de p
      //
      position = gti320::Icp::nearestNeighbor(result, m_refPoints);

      // TODO Étape 1.3 affecter B[:,i] = p and A[:,i] = m_refPoints[:,k]
      //
      B=B.assignPoint3d(i, result(0), result(1), result(2));
      //B(0, i) = result(0);
      //B(1, i) = result(1);
      //B(2, i) = result(2);
      A=A.assignPoint3d(i, m_refPoints(0, position), m_refPoints(1, position), m_refPoints(2, position));
      //A(0, i) = m_refPoints(0, position);
      //A(1, i) = m_refPoints(1, position);
      //A(2, i) = m_refPoints(2, position);
  ...
  // l'ajouter à m_srcTM
  //
  tmResult = gti320::Icp::bestFitTransform(B, A);
  m_srcTM = tmResult* m_srcTM;
